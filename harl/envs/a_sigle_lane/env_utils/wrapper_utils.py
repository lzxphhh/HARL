...
def analyze_traffic(state, lane_ids):
    ...
    for lane_id, stats in lane_statistics.items():
        ...
        if len(veh_stats) == 0:
            ...
            vehs_stats = np.pad(veh_stats, ((0, 15 - len(veh_stats)), (0, 0, 0, 0, 0, 0)), 'constant', constant_values=0)
        else:
...
def compute_base_ego_vehicle_features(
    ...
    for ego_id, ego_info in ego_statistics.items():
        # ############################## 自己车的信息 ############################## 13
        position, speed, acceleration, heading, road_id, lane_index, surroundings, surroundings_expand = ego_info
        # 速度归一化  - 1
        normalized_speed = speed / state_divisors[2]
        # 加速度归一化  - 1
        normalized_acceleration = acceleration / state_divisors[3]
        ...
        # One-hot encode road_id - 5
        road_id_one_hot = one_hot_encode(road_id, unique_edges)
        if position_y < 2 and heading <= 90:
            next_node[ego_id] = [1, 0]
            dis_next_node = 1 - normalized_position_x
        elif position_x > 198 and 90 < heading <= 180:
            next_node[ego_id] = [1, 1]
            dis_next_node = 1 - normalized_position_y
        elif position_y > 198 and 180 < heading <= 270:
            next_node[ego_id] = [0, 1]
            dis_next_node = normalized_position_x
        else:
            next_node[ego_id] = [0, 0]
        ...
        for index, (_, statistics) in enumerate(surroundings.items()):
            ...
            surround.append([veh_type, relat_x, relat_y, relat_v, veh_acc, veh_heading])  # relat_x, relat_y, relat_v
        ...
        flat_surround = [item for sublist in surround for item in sublist]
        # 如果周围车辆的信息不足2*6个, 补0 对齐到最多数量的周车信息
        if len(flat_surround) < 12:
            flat_surround += [0] * (12 - len(flat_surround))
    ...
    for ego_id in ego_statistics.keys():
        ...
        feature_vectors_current[ego_id]['next_node'] = next_node[ego_id][0]
        feature_vectors_current[ego_id]['dis_next_node'] = next_node[ego_id][1]
