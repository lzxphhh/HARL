...
class VehEnvWrapper(gym.Wrapper):
    ...
    def __init__(self, env: Env,
        ...
        if self.strategy == 'base':
            ...
            # dist_next_node: dist - 1
            # vehicles_state (ego + front + rear): pos_x, pos_y, speed, acceleration, heading - 3*5
            # ego_lane_statistics: num_veh, lane_length, density, mean_speed, mean_acceleration, CAV_penetration - 6
            self.self_obs_size = 8 + 2 + 1 + 3 * 5 + 6
                ...
                if front_vehicle not in [None, ()] and front_vehicle[0] != '':  # 有可能是空的
                    ...
                    if front_vehicle_lane_index != ego_lane_index:
                        ...
                        if front_vehicle_expand not in [None, ()] and front_vehicle_expand[0] != '':
                            ...
                            if front_vehicle[0][:3] == 'HDV':
                                long_dist = front_vehicle_expand[1] + 1.5
                            elif front_vehicle[0][:3] == 'CAV':
                                long_dist = front_vehicle_expand[1] + 1.0
                            ...
                            if front_vehicle_expand_exp not in [None, ()] and front_vehicle_expand_exp[0] != '':
                                ...
                                if front_vehicle_expand_exp[0][:3] == 'HDV':
                                    ...
                                    expand_veh_type = 1
                                if front_vehicle_expand[0][:3] == 'HDV':
                                    long_dist = front_vehicle_expand_exp[1] + 1.5
                                elif front_vehicle_expand[0][:3] == 'CAV':
                                    long_dist = front_vehicle_expand_exp[1] + 1.0
                                else:
                                    raise ValueError('Unknown vehicle type')
                    ...
                    if back_vehicle_lane_index != ego_lane_index:
                        ...
                        if back_vehicle_expand not in [None, ()] and back_vehicle_expand[0] != '':
                            ...
                            if back_vehicle_expand[0][:3] == 'HDV':
                                expand_veh_type = 0
                                long_dist = back_vehicle_expand[1] + 1.5
                            elif back_vehicle_expand[0][:3] == 'CAV':
                                expand_veh_type = 1
                                long_dist = back_vehicle_expand[1] + 1.0
                            else:
                                raise ValueError('Unknown vehicle type')
                            ...
                            if back_vehicle_expand_exp not in [None, ()] and back_vehicle_expand_exp[0] != '':
                                ...
                                if back_vehicle_expand_exp[0][:3] == 'HDV':
                                    expand_veh_type = 0
                                    long_dist = back_vehicle_expand_exp[1] + 1.5
                                elif back_vehicle_expand_exp[0][:3] == 'CAV':
                                    expand_veh_type = 1
                                    long_dist = back_vehicle_expand_exp[1] + 1.0
                                else:
                                    raise ValueError('Unknown vehicle type')
        ...
        for _veh_id in raw_action:
            if _veh_id in self.actions:  # 只更新 ego vehicle 的速度
                ...
                if raw_action[_veh_id] in range(0, 7):
                    self.action_command = (0, self.current_speed[_veh_id]+self.action_pointer[raw_action[_veh_id]])
                else:
