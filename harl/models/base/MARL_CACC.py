...
class CACC_rep(nn.Module):
    def __init__(self, obs_dim, action_dim, n_embd, action_type='Discrete', args=None):
        ...
        self.mlp_local_combined = MLPBase(args, [5 + 2 + 64 * 1])

        # 定义模板结构，使用float类型的张量来节省内存
        self.example_extend_info = {
            'road_structure': torch.zeros(2, dtype=torch.float32),               # 0
            'next_node': torch.zeros(3, dtype=torch.float32),                    # 1
            'self_stats': torch.zeros(8, dtype=torch.float32),                  # 2
    ...
    def forward(self, obs, batch_size=20):
        ...
        ego_last_action = info_current[2][:, 6:8]
        ...
        surround_hist_4 = info_current[5]
        surround_hist_6 = info_current[6]
        veh_hist = torch.cat((ego_hist, surround_hist_4), dim=1)

        # 使用预先分配的邻接矩阵，减少每次计算时的内存开销
        if not hasattr(self, 'adj') or self.adj.size(0) != batch_size:
            self.adj = torch.ones(batch_size, 5, 5, device=obs.device)
            self.adj[:, 1:, 1:] = 0
        ...
        local_combined_embedding = torch.cat((road_static_info, ego_last_action, ego_relation), dim=1)
